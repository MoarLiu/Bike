...
- 登录后的 Web 端可以访问文档级同步 API；其他客户端可使用配置中的设备同步密钥。
...
只有显式设置 `"sync": { "enabled": true }` 时才会启动同步服务；旧配置不包含 `sync` 字段时，同步 API 保持关闭。同步数据保存在 SQLite 文件中。Web 前端同源登录后可直接手动同步；移动端等其他客户端需要配置设备密钥。同步设置中输入的设备密钥会明文保存在本机应用配置中，不读取系统密钥串。
