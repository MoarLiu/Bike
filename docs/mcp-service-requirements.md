...
- 只读 MVP 的实现成本和风险都较低，适合先验证真实使用价值。
...
写入能力不进入 MVP，但需要提前约束接口形态。

### 11.1 写入原则

- 默认关闭，需要在配置里显式启用。
- 每次写入前创建快照。
- 每次写入要求传入 `expectedRevision`。
- 支持 `dryRun`，dry-run 返回变更预览，不落盘。
- 删除操作必须软删除或进入回收记录，不应直接不可恢复删除。
...
- 后续写入如果不经过应用内事务，容易和 IndexedDB 状态冲突。
...
- 写入能力是否要等 LocalOutline 有版本历史/回收站后再做？
