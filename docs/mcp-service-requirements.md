...
日期：2026-06-04
适用产品：Bike 1.2.0 起；写入能力目标版本为 1.3.0

## 1. 背景

Bike 是一个本地优先的大纲、脑图和演示工具，核心数据是一份 `Workspace`，其中包含多篇 `OutlineDocument`，每篇文档由树形 `OutlineNode` 组成。当前数据主要保存在浏览器 IndexedDB，桌面版可备份到 iCloud Drive 的 `bike-workspace.json`。

随着 Codex、Claude 等 AI agent 逐渐成为日常写作、编码和知识整理入口，Bike 可以通过 MCP 服务把本地大纲暴露为可读取、可搜索、可定位、可导出的结构化上下文。这样 agent 不需要用户手动复制整篇大纲，也不需要把无关内容塞进上下文，而是可以按需读取文档、节点和子树。
...
- Bike 从“大纲编辑器”升级为“AI agent 可调用的结构化知识库”。
...
- 提供少量 prompt 模板，帮助 agent 更稳定地使用 Bike 内容。
...
用户在 Bike 里维护一个产品需求或技术方案大纲。使用 Codex 时，用户说：“读取 Bike 里的 MCP 服务需求，帮我拆实现任务。”Codex 通过 MCP 搜索相关文档，读取文档树，然后生成实现步骤。
...
用户在 Claude 里要求：“总结 Bike 里某个节点下面的所有内容。”Claude 通过 `get_node` 获取该节点、父级路径和指定深度的子节点，输出摘要。
...
用户让 AI 根据一篇 Bike 文档生成 PRD、开发任务、测试清单或发布说明。AI 使用 `get_document` 或 `export_document` 获取 Markdown，再按照 prompt 模板输出。
...
- Streamable HTTP：适合 Bike 应用运行时暴露本地端口。
...
Bike 当前权威数据在浏览器 IndexedDB 中，外部 Node.js MCP 进程无法直接稳定读取浏览器 IndexedDB。桌面版已有 iCloud Drive 备份文件；Electron 版默认位置是：
...
- 写入操作进入应用内事务，由 Bike UI 负责合并、保存和提示冲突。
...
```json
{
  ...
  "workspacePath": "/absolute/path/to/bike-workspace.json"
}
```
...
bike://documents
bike://document/{documentId}
bike://document-markdown/{documentId}
bike://node/{documentId}/{nodeId}
```

资源要求：

- `bike://workspace/summary` 返回工作区摘要。
- `bike://documents` 返回文档列表。
- `bike://document/{documentId}` 返回 compact JSON。
- `bike://document-markdown/{documentId}` 返回 Markdown。
- `bike://node/{documentId}/{nodeId}` 返回节点上下文。
...
写入能力作为 1.3.0 主需求进入实施范围。目标是在不破坏本地优先和数据安全边界的前提下，让 Codex、Claude 等 agent 可以对 Bike 工作区执行小范围、可预览、可回滚的结构化变更。
...
快照文件名建议包含时间戳和操作名，例如 `bike-mcp-20260604-153000-update-node.json`。
...
- macOS Electron 备份路径：`~/Library/Mobile Documents/com~apple~CloudDocs/Bike/bike-workspace.json`
- macOS Swift 原生版备份路径：`~/Library/Mobile Documents/com~apple~CloudDocs/Bike/.backups/bike-workspace.json`
- 项目开发路径：通过环境变量显式指定。
...
- 增加 npm script，例如 `npm run mcp:bike`。
...
- 可以通过 stdio 启动 Bike MCP server。
- 可以从配置路径读取 `bike-workspace.json`。
...
- `BIKE_MCP_MODE=write` 或配置文件 `mode: "write"` 可启用受控写入。
...
- [Bike 本地优先架构设计](./local-first-architecture.md)
- [Bike 产品分析](./product-analysis.md)
